...
public partial class ViewGradeOnline : ComponentBase
{
    ...
    private async Task GetGrade()
    {
        ...
        var serverNow = DateTime.Now; // Hora actual del servidor (UTC-5)

        // Crear la fecha objetivo en la zona horaria de Nicaragua (UTC-6)
        var targetDateInNicaragua = new DateTime(2025, 4, 10, 6, 0, 0); // 10/4/2025 6:00 AM UTC-6

        // Convertir la hora de Nicaragua a la hora del servidor (sumar 1 hora)
        var targetDateInServerTime = targetDateInNicaragua.AddHours(1); // 10/4/2025 7:00 AM UTC-5

        if (serverNow >= targetDateInServerTime)
        {
            ErrorMessage = "Las calificaciones estarán disponibles el día 11 de abril de 2025.";
